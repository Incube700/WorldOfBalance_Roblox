...
- VFX templates: `CombatVfxService` clones templates from `ReplicatedStorage/Shared/Assets/VFX`; `VfxConfig` is grouped as `Shot`, `Impact`, `Ricochet`, `DeathExplosion`, and `BurningTank`. `Impact.WallImpact`, `DamageHit`, `NoPen`, and `SelfHit` now map separate combat outcomes to readable defaults. Setup notes: `docs/VFX_TEMPLATE_SETUP.md`; tuning audit: `docs/VFX_TUNING_PASS.md`.
...
- VFX template collection: run `docs/patches/COLLECT_AND_INSTALL_VFX_TEMPLATES_COMMAND.lua` outside Play Mode to collect known Workspace donors into `ReplicatedStorage/Shared/Assets/VFX`, sanitize scripts/collision/query, and move direct Workspace donors into `Workspace/WOB_EditorOnly_AssetDonors`. Run `docs/patches/PREVIEW_VFX_TEMPLATES_COMMAND.lua` outside Play Mode to preview installed templates at `Workspace/WOB_Generated/VFXPreview`.
...
- Optional VFX: muzzle/impact/ricochet/burning templates are quiet if absent. Death plays `TankExplosionTemplate` with procedural fallback plus optional `TankBurningTemplate`; wall/armor ricochets try `RicochetTemplate` before `Impact.WallImpact` fallback.
