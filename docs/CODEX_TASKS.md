...
- VFX template collection: run `docs/patches/COLLECT_AND_INSTALL_VFX_TEMPLATES_COMMAND.lua` outside Play Mode to collect known Workspace donors into `ReplicatedStorage/Shared/Assets/VFX`, sanitize scripts/collision/query, and move direct Workspace donors into `Workspace/WOB_EditorOnly_AssetDonors`.
...
- Optional VFX: `VfxConfig.BurningTank` and `VfxConfig.Ricochet` are enabled but quiet if their templates are absent. Death plays explosion plus optional burning; wall/armor ricochets try `RicochetTemplate` before impact fallback.
